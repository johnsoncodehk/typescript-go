...
var textToKeyword = collections.NewFrozenMap(map[string]ast.Kind{
...
var textToToken = func() *collections.FrozenMap[ast.Kind] {
	...
	maps.Insert(m, textToKeyword.All())
	return collections.NewFrozenMap(m)
}()
...
func GetIdentifierToken(str string) ast.Kind {
	if len(str) >= 2 && len(str) <= 12 && str[0] >= 'a' && str[0] <= 'z' {
		keyword := textToKeyword.GetOrZero(str)
...
var tokenToText = func() [ast.KindCount]string {
	...
	for text, kind := range textToToken.All() {
		result[kind] = text
	}
	return result
}()
...
func GetViableKeywordSuggestions() []string {
	result := make([]string, 0, textToKeyword.Len())
	for text := range textToKeyword.Keys() {
		if len(text) > 2 {
