...
export const depaware = task({
    ...
    hiddenFromTaskList: true,
    run: async () => {
        let { stdout } = await $pipe`go tool depaware ./cmd/tsgo`;
        stdout = stdout
            .split("\n")
            .filter(line => !/^\s+github\.com\/microsoft\/typescript-go/.test(line))
            .map(line => line.replace(/\s+from .*$/, ""))
            .join("\n");
        stdout = stdout.replace(
            /\(generated by github.com\/tailscale\/depaware\)/,
            "(generated by github.com/tailscale/depaware and Herebyfile.mjs)",
        );
        stdout = stdout.trim() + "\n";
        const out = path.join(__dirname, "cmd", "tsgo", "depaware.txt");
