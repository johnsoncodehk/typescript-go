...
func main() {
	...
	for _, pkg := range pkgs {
		...
		// Filter out -dir from the generated comment since it contains machine-specific paths.
		var commentArgs []string
		for i, arg := range os.Args[1:] {
			if arg == "-dir" || arg == "--dir" {
				continue // skip -dir
			}
			if i > 0 && (os.Args[i] == "-dir" || os.Args[i] == "--dir") {
				continue // skip -dir's value
			}
		...
		} else if *pkgDir != "" && !filepath.IsAbs(outputName) {
			// When -dir is specified, resolve -output relative to it.
			outputName = filepath.Join(dir, outputName)
		}
...
func loadPackages(
	...
	// resolves the package in the correct module context. This is important
	// when the stringer binary is built from a different module than the
	// package being analyzed (e.g., when using go run -C).
	if len(patterns) == 1 {
